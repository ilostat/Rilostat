...
More advanced users can take advantage of ILOSTAT's well-structured [bulk download facility](https://ilostat.ilo.org/data/bulk/) ([user guide](https://www.ilo.org/ilostat-files/Documents/ILOSTAT_BulkDownload_Guidelines.pdf)).
...
```{r, eval=TRUE}
toc <-  dplyr::filter(get_ilostat_toc(), collection == 'STI', freq == 'M', quiet = TRUE)
```
...
```{r, eval=TRUE}
dat <- get_ilostat(id = 'TRU_TTRU_SEX_AGE_NB_M', time_format = 'date', quiet = TRUE) 
```
...
dat <- get_ilostat(id = 'UNE_2EAP_SEX_AGE_RT_A', filters = list(
												...
												detail = 'serieskeysonly', quiet = TRUE)
```
